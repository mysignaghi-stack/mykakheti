...
export default function AdminAgroPanel() {
  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    ...
    if (!editItem) return;
    setSaving(true);
    const payload = { price: newPrice, details: editDetails };
    try {
      const targetId = !isNaN(Number((editItem as any).id)) ? Number((editItem as any).id) : (editItem as any).id;
      const { error } = await (supabase as any).from('agro_prices').update(payload).eq('id', targetId);
      if (error) throw error;
      await fetchItems();
      setEditItem(null);
