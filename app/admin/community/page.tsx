      ...
      <div className="max-w-5xl mx-auto">
        <div className="flex gap-2 mb-6">
          {[
            ...
            <button key={t.key} onClick={() => { setTab(t.key); loadPending(t.key); loadApproved(t.key); }} className={`px-3 py-2 rounded-xl text-xs border ${tab===t.key?'bg-amber-600 text-white border-amber-600':'bg-white/5 text-white border-white/10'}`}>{t.label}</button>
          ))}
        </div>
